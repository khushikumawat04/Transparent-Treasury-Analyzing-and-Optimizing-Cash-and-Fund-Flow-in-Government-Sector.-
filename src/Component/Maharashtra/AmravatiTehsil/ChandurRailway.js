...
const ChandurRailway = () => {
  ...
  const getvillagedata = SectorData.find((state) => state.state_id === "2")
        .District.find((district) => district.District_id === "3")
        .tehsil.find((tehsil) => tehsil.tehsil_id === "5").villages;
...
<Link to="/sector"><span><AiOutlineArrowLeft /></span></Link>
    ...
    <div className='tehsil-sub-text'>
        <h1>total budget</h1>
        <h4>$ 20,00,000</h4>
        <p>According to census 2011 information the sub-district code of Chandur Railway Block (CD) is 04013. Total area of chandur railway tehsil is 562 km² including 559.30 km² rural area and 2.54 km² urban area. Chandur Railway tehsil has a population of 96,907 peoples, out of which urban population is 19,776 while rural population is 77,131. Chandur Railway tehsil has a population density of 172.5 inhabitants per square kilometre. There are about 23,606 houses in the sub-district, including 4,337 urban houses and 19,269 rural houses.</p>
        </div>
    </div>
    ...
    <div className="tehsil-table">
        <div className="button">
           ...
           <button><CSVLink
      data={userdata}
      filename="Chandur RailwayVillage"
      style={{color:"white"}}
