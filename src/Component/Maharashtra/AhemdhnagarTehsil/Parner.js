...
const Parner = () => {
  ...
  const getvillagedata = SectorData.find((state) => state.state_id === "2")
        ...
        .tehsil.find((tehsil) => tehsil.tehsil_id === "8").villages;
...
<Link to="/sector"><span><AiOutlineArrowLeft /></span></Link>
    ...
    <div className='tehsil-sub-text'>
        <h1>total budget</h1>
        <h4>$ 20,00,000</h4>
        <p> Parner tehsil has a population of 2,74,167 peoples. Parner tehsil has a population density of 147.7 inhabitants per square kilometre. There are about 56,476 houses in the sub-district. When it comes to literacy, 67.22% population of parner tehsil is literate, out of which 74.29% males and 59.81% females are literate.</p>
        </div>
    </div>
